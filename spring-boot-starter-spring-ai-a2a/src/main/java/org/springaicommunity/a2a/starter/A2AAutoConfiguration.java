 ...
 * <li>Create an {@link A2AAgentServer} bean if a {@link SpringAIAgentExecutor} is present</li>
 ...
 *
 * <p>
 * Users only need to provide a {@link SpringAIAgentExecutor} bean and the starter will automatically
...
public class A2AAutoConfiguration {
	...
	private static final Logger logger = LoggerFactory.getLogger(A2AAutoConfiguration.class);

	/**
	 * Create the AgentCard bean from configuration properties if not already defined.
	...
	public AgentCard agentCard(A2AProperties properties, @Value("${server.port:8080}") int serverPort) {
		...
		if (!StringUtils.hasText(agentProps.getName())) {
			throw new IllegalArgumentException(
					"spring.ai.a2a.agent.name must be configured. Please set this property in your application configuration.");
		}

		if (!StringUtils.hasText(agentProps.getDescription())) {
			throw new IllegalArgumentException(
					"spring.ai.a2a.agent.description must be configured. Please set this property in your application configuration.");
		}

		String basePath = properties.getServer().getBasePath();
		String agentUrl = "http://localhost:" + serverPort + basePath;

		logger.info("Creating AgentCard for agent '{}' with URL: {}", agentProps.getName(), agentUrl);
	...
	/**
	 * Create the A2A Agent Server bean if a SpringAIAgentExecutor is present.
	 * <p>
	 * This enforces that Spring AI applications must use {@link SpringAIAgentExecutor}
	...
	@Bean
	@ConditionalOnMissingBean
	@ConditionalOnBean(SpringAIAgentExecutor.class)
	public A2AAgentServer a2aAgentServer(AgentCard agentCard, SpringAIAgentExecutor agentExecutor) {
		logger.info("Creating A2AAgentServer for agent: {}", agentCard.name());
